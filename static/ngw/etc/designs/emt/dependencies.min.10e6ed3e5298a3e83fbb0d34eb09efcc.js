...
if((a[0]<2&&a[1]<9)||(a[0]==1&&a[1]==9&&a[2]<1)||(a[0]>3)){throw new Error("Bootstrap's JavaScript requires jQuery version 1.9.1 or higher, but lower than version 4")
}}(jQuery);
...
var animate=(function(){return{init:function(){a()
}};
function a(){function b(d,e,f){d.removeClass("animation");
d.css("visibility","hidden");
d.addClass("animated");
d.addClass(e);
d.css("animation-duration",f);
d.css("visibility","visible")
}function c(){$(".animation").each(function(){var d=isElementOnViewPort($(this));
if(d){b($(this),"fadeInUp","1s")
}else{$(this).removeClass("animated fadeInUp")
}});
$(".buttonanimation").each(function(){var d=isElementOnViewPort($(this));
if(d){b($(this),"zoomIn","1s")
}else{$(this).removeClass("animated zoomInUp")
}})
}c();
 ...
 */
;
(function(d,f){var e=d.jQuery||d.Zepto,a=0,b=false;
e.fn.Lazy=e.fn.lazy=function(h){return new g(this,h)
};
e.Lazy=e.lazy=function(o,h,q){if(e.isFunction(h)){q=h;
h=[]
}if(!e.isFunction(q)){return
}o=e.isArray(o)?o:[o];
h=e.isArray(h)?h:[h];
var j=g.prototype.config,r=j._f||(j._f={});
for(var m=0,k=o.length;
m<k;
m++){if(j[o[m]]===f||e.isFunction(j[o[m]])){j[o[m]]=q
}}for(var n=0,p=h.length;
n<p;
n++){r[h[n]]=o[0]
}};
function c(F,G,x,h,t){var z=0,B=-1,n=-1,w=false,C="afterLoad",J="load",m="error",k="img",r="src",j="srcset",y="sizes",v="background-image";
function H(){w=d.devicePixelRatio>1;
x=D(x);
if(G.delay>=0){setTimeout(function(){i(true)
},G.delay)
}if(G.delay<0||G.combined){h.e=p(G.throttle,function(K){if(K.type==="resize"){B=n=-1
}i(K.all)
});
h.a=function(K){K=D(K);
x.push.apply(x,K)
};
h.g=function(){return(x=e(x).filter(function(){return !e(this).data(G.loadedName)
}))
};
h.f=function(K){for(var L=0;
L<K.length;
L++){var M=x.filter(function(){return this===K[L]
});
if(M.length){i(false,M)
}}};
i();
e(G.appendScroll).on("scroll."+t+" resize."+t,h.e)
}}function D(P){var K=G.defaultImage,S=G.placeholder,R=G.imageBase,N=G.srcsetAttribute,Q=G.loaderAttribute,U=G._f||{};
P=e(P).filter(function(){var X=e(this),W=l(this);
return !X.data(G.handledName)&&(X.attr(G.attribute)||X.attr(N)||X.attr(Q)||U[W]!==f)
}).data("plugin_"+G.name,F);
for(var O=0,L=P.length;
O<L;
O++){var M=e(P[O]),V=l(P[O]),T=M.attr(G.imageBaseAttribute)||R;
if(V===k&&T&&M.attr(N)){M.attr(N,o(M.attr(N),T))
}if(U[V]!==f&&!M.attr(Q)){M.attr(Q,U[V])
}if(V===k&&K&&!M.attr(r)){M.attr(r,K)
}else{if(V!==k&&S&&(!M.css(v)||M.css(v)==="none")){M.css(v,"url('"+S+"')")
}}}return P
}function i(R,V){if(!x.length){if(G.autoDestroy){F.destroy()
}return
}var K=V||x,M=false,T=G.imageBase||"",O=G.srcsetAttribute,Q=G.handledName;
for(var P=0;
P<K.length;
P++){if(R||V||q(K[P])){var N=e(K[P]),W=l(K[P]),L=N.attr(G.attribute),U=N.attr(G.imageBaseAttribute)||T,S=N.attr(G.loaderAttribute);
if(!N.data(Q)&&(!G.visibleOnly||N.is(":visible"))&&((L||N.attr(O))&&((W===k&&(U+L!==N.attr(r)||N.attr(O)!==N.attr(j)))||(W!==k&&U+L!==N.css(v)))||S)){M=true;
N.data(Q,true);
A(N,W,U,S)
}}}if(M){x=e(x).filter(function(){return !e(this).data(Q)
})
}}function A(Q,Y,T,U){++z;
var R=function(){u("onError",Q);
E();
R=e.noop
};
u("beforeLoad",Q);
var W=G.attribute,P=G.srcsetAttribute,N=G.sizesAttribute,L=G.retinaAttribute,S=G.removeAttribute,K=G.loadedName,X=Q.attr(L);
if(U){var O=function(){if(S){Q.removeAttr(G.loaderAttribute)
}Q.data(K,true);
u(C,Q);
setTimeout(E,1);
O=e.noop
};
Q.off(m).one(m,R).one(J,O);
if(!u(U,Q,function(Z){if(Z){Q.off(J);
O()
}else{Q.off(m);
R()
}})){Q.trigger(m)
}}else{var M=e(new Image());
M.one(m,R).one(J,function(){Q.hide();
if(Y===k){Q.attr(y,M.attr(y)).attr(j,M.attr(j)).attr(r,M.attr(r))
}else{Q.css(v,"url('"+M.attr(r)+"')")
}Q[G.effect](G.effectTime);
if(S){Q.removeAttr(W+" "+P+" "+L+" "+G.imageBaseAttribute);
if(N!==y){Q.removeAttr(N)
}}Q.data(K,true);
u(C,Q);
M.remove();
E()
});
var V=(w&&X?X:Q.attr(W))||"";
M.attr(y,Q.attr(N)).attr(j,Q.attr(P)).attr(r,V?T+V:null);
M.complete&&M.trigger(J)
}}function q(N){var O=N.getBoundingClientRect(),P=G.scrollDirection,K=G.threshold,M=((I()+K)>O.top)&&(-K<O.bottom),L=((s()+K)>O.left)&&(-K<O.right);
if(P==="vertical"){return M
}else{if(P==="horizontal"){return L
}}return M&&L
}function s(){return B>=0?B:(B=e(d).width())
}function I(){return n>=0?n:(n=e(d).height())
}function l(K){return K.tagName.toLowerCase()
}function o(N,O){if(O){var K=N.split(",");
N="";
for(var M=0,L=K.length;
M<L;
M++){N+=O+K[M].trim()+(M!==L-1?",":"")
}}return N
}function p(L,N){var M,K=0;
return function(Q,P){var O=+new Date()-K;
function R(){K=+new Date();
N.call(F,Q)
}M&&clearTimeout(M);
if(O>L||!G.enableThrottle||P){R()
}else{M=setTimeout(R,L-O)
}}
}function E(){--z;
if(!x.length&&!z){u("onFinishedAll")
}}function u(M,L,K){if((M=G[M])){M.apply(F,[].slice.call(arguments,1));
return true
}return false
}if(G.bind==="event"||b){H()
}else{e(d).on(J+"."+t,H)
}}function g(l,j){var h=this,k=e.extend({},h.config,j),m={},i=k.name+"-"+(++a);
h.config=function(n,o){if(o===f){return k[n]
}k[n]=o;
return h
};
h.addItems=function(n){m.a&&m.a(e.type(n)==="string"?e(n):n);
return h
};
h.getItems=function(){return m.g?m.g():{}
};
h.update=function(n){m.e&&m.e({},!n);
return h
};
h.force=function(n){m.f&&m.f(e.type(n)==="string"?e(n):n);
return h
};
h.loadAll=function(){m.e&&m.e({all:true},true);
return h
};
h.destroy=function(){e(k.appendScroll).off("."+i,m.e);
e(d).off("."+i);
m={};
return f
};
c(h,k,l,m,i);
return k.chainable?l:h
}g.prototype.config={name:"lazy",chainable:true,autoDestroy:true,bind:"load",threshold:500,visibleOnly:false,appendScroll:d,scrollDirection:"both",imageBase:null,defaultImage:"data:image/gif;base64,R0lGODlhAQABAIAAAP///wAAACH5BAEAAAAALAAAAAABAAEAAAICRAEAOw==",placeholder:null,delay:-1,combined:false,attribute:"data-src",srcsetAttribute:"data-srcset",sizesAttribute:"data-sizes",retinaAttribute:"data-retina",loaderAttribute:"data-loader",imageBaseAttribute:"data-imagebase",removeAttribute:true,handledName:"handled",loadedName:"loaded",effect:"show",effectTime:0,enableThrottle:true,throttle:250,beforeLoad:f,afterLoad:f,onError:f,onFinishedAll:f};
e(d).on("load",function(){b=true
})
})(window);
var objectFitVideos=function(d){var j=navigator.userAgent.indexOf("Edge/")>=0;
var b=new Image(),g="object-fit" in b.style&&!j,c="object-position" in b.style&&!j,a=/(object-fit|object-position)\s*:\s*([-\w\s%]+)/g;
if(!g||!c){e(d);
k("resize","optimizedResize")
}function i(m){var o=getComputedStyle(m).fontFamily,l=null,n={};
while((l=a.exec(o))!==null){n[l[1]]=l[2]
}if(n["object-position"]){return h(n)
}return n
}function e(m){var l=-1;
if(!m){m=document.querySelectorAll("video")
}else{if(!("length" in m)){m=[m]
}}while(m[++l]){var n=i(m[l]);
if(n["object-fit"]||n["object-position"]){n["object-fit"]=n["object-fit"]||"fill";
f(m[l],n)
}}}function f(p,r){if(r["object-fit"]==="fill"){return
}var m=p.style,n=window.getComputedStyle(p);
var l=document.createElement("object-fit");
l.appendChild(p.parentNode.replaceChild(l,p));
var s={height:"100%",width:"100%",boxSizing:"content-box",display:"inline-block",overflow:"hidden"};
"backgroundColor backgroundImage borderColor borderStyle borderWidth bottom fontSize lineHeight left opacity margin position right top visibility".replace(/\w+/g,function(t){s[t]=n[t]
});
for(var o in s){l.style[o]=s[o]
}m.border=m.margin=m.padding=0;
m.display="block";
m.opacity=1;
p.addEventListener("loadedmetadata",q);
window.addEventListener("optimizedResize",q);
if(p.readyState>=1){p.removeEventListener("loadedmetadata",q);
q()
}function q(){var t=window.innerWidth;
var u=window.innerHeight;
if(t/16>=u/9){m.width=t+"px";
m.height="auto"
}else{m.width="auto";
m.height=u+"px"
}if(p.autoplay){p.play()
}}}function h(l){if(~l["object-position"].indexOf("left")){l["object-position-x"]="left"
}else{if(~l["object-position"].indexOf("right")){l["object-position-x"]="right"
}else{l["object-position-x"]="center"
}}if(~l["object-position"].indexOf("top")){l["object-position-y"]="top"
}else{if(~l["object-position"].indexOf("bottom")){l["object-position-y"]="bottom"
}else{l["object-position-y"]="center"
}}return l
}function k(o,n,r){r=r||window;
var m=false,l=null;
try{l=new CustomEvent(n)
}catch(q){l=document.createEvent("Event");
l.initEvent(n,true,true)
}var p=function(){if(m){return
}m=true;
requestAnimationFrame(function(){r.dispatchEvent(l);
m=false
})
};
r.addEventListener(o,p)
}};
